...
RADAR_KEYS = [("Jump Height", "CMJ jump ht"), ("IMTP Relative Force", "IMTP N/kg"),
              ...
              ("__QUAD__", "Quad ISO"), ("20 m sprint", "20 m sprint")]
...
def _avg_spoke(rm, left, right, unit):
    ...
    return dict(value=v, target=g, status=worst, unit=unit, dir="Higher")
...
def _spoke(name, rm):
    ...
    return dict(value=v, target=g, status=r["status"], unit=r["unit"],
                dir=(r.get("norm") or {}).get("dir", "Higher"))
...
def radar_svg(groups):
    ...
    for name, label in RADAR_KEYS:
        ...
        if s:
            s["label"] = label
            if s.get("dir") == "Lower" and s["value"]:
                s["score"] = s["target"] / s["value"]
            else:
                s["score"] = s["value"] / s["target"]
            spokes.append(s)
    if len(spokes) < 3:
        return ""
