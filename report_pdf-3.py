...
# Friendly short axis labels; anything not listed falls back to the metric name.
SHORT_LABEL = {
    "Jump Height": "CMJ jump ht", "Peak Power": "Peak power", "CMJ Peak Force": "CMJ peak F",
    "RSI-modified": "RSI-mod", "IMTP Peak Force": "IMTP peak F", "IMTP Relative Force": "IMTP N/kg",
    "IMTP RFD 0\u2013200 ms": "IMTP RFD", "Hop RSI (best)": "Hop RSI", "Hop Jump Height": "Hop jump ht",
    "Adductor Peak Force": "Hip ADD", "Abductor Peak Force": "Hip ABD",
    "Nordic Relative Force": "Nordic N/kg",
    "10 m sprint": "10 m", "20 m sprint": "20 m", "30 m sprint": "30 m",
    "5-10-5 (pro-agility)": "5-10-5",
    "__NORDIC__": "Nordic (L/R)", "__QUAD__": "Quad ISO (L/R)",
}
...
def _score(value, target, dirn):
    """Radar spoke score, direction-aware. >=1 means at/beyond target (further out = better)."""
    if not target:
        return None
    if dirn == "Lower":
...
def radar_options(groups):
    """Return [(key, label), ...] of metrics that can be plotted on the radar
    (a numeric result plus a Higher/Lower green target). Fed to the app's picker."""
    rm = _rowmap(groups)
    opts = []
...
def render_pdf(meta, population, groups, counts, prios, radar_keys=None):
    ...
    *{{box-sizing:border-box;font-family:'Liberation Sans','DejaVu Sans',sans-serif;}}
    ...
    radar_block = (f'<div class="sec">Athlete profile \u2014 key components</div>'
                   ...
                   f'(dashed green ring). Dots show status.</div>') if rsvg else ""
...
def render_ham_pdf(meta, phase, groups, counts, disclaimer=""):
    ...
    *{{box-sizing:border-box;font-family:'Liberation Sans','DejaVu Sans',sans-serif;}}
    ...
    .grp{{background:{GBAND};color:#fff;font-weight:700;font-size:9.5px;letter-spacing:.4px;padding:3px 8px;margin-top:7px;border-radius:3px;break-inside:avoid;break-after:avoid;}}
    .grp::before{{content:"";display:inline-block;width:7px;height:7px;background:{BLUE};border-radius:2px;margin-right:7px;}}
    .row{{display:grid;grid-template-columns:188px 56px 1fr 118px;align-items:center;gap:8px;padding:5px 6px;border-bottom:1px solid {LINE};break-inside:avoid;}}
    .mname{{font-weight:700;font-size:10px;}} .unit{{color:{MUTE};font-weight:400;font-size:8.5px;margin-left:5px;}}
    .mval{{font-weight:800;font-size:13px;text-align:center;color:{BLACK};}}
    ...
    <div class="hd"><div class="brand">{icon_html}<div class="name">BASE <b>HEALTH</b><span>NOOSA</span></div></div>
      <div class="ttl"><h1>Hamstring Rehab &amp; Return-to-Play</h1>
      <div class="sub">Injured-limb tracking vs phase targets \u2022 change-vs-previous</div></div></div>
    <div class="meta">
      <span><b>Athlete</b> {esc(meta.get('name'))}</span><span><b>Date</b> {esc(meta.get('date'))}</span>
      <span><b>Injured side</b> {esc(meta.get('injured'))}</span><span><b>Clinician</b> {esc(meta.get('clinician'))}</span>
      <span><b>Wks since injury</b> {esc(meta.get('weeks'))}</span><span><b>Sport</b> {esc(meta.get('sport'))}</span>
      <span><b>Notes</b> {esc(meta.get('notes'))}</span></div>
    <div class="band"><div>Rehab phase <span class="pop">{esc(phase)}</span></div>
